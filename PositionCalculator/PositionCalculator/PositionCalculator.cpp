...
const float tolerance = 1.000f; // Threshold to consider gravity effect negligible
    ...
    for (auto& sat : satellites) {
        for (int step = 0; step < numberOfSteps; step++) {
            ...
            for (const auto& planet : planetoids) {
                ...
                if (distance > tolerance) { // Prevent division by zero
            ...
            if (step < numberOfSteps-1) { // Update velocity and position for the next step
                ...
                sat.steps[step + 1].velocity[0] = sat.steps[step].acceleration[0] * timestep;
                sat.steps[step + 1].velocity[1] = sat.steps[step].acceleration[1] * timestep;
                sat.steps[step + 1].velocity[2] = sat.steps[step].acceleration[2] * timestep;
                
                sat.steps[step + 1].position[0] = sat.steps[step].velocity[0] * timestep + 0.5 * sat.steps[step].acceleration[0] * timestep * timestep;
                sat.steps[step + 1].position[1] = sat.steps[step].velocity[1] * timestep + 0.5 * sat.steps[step].acceleration[1] * timestep * timestep;
                sat.steps[step + 1].position[2] = sat.steps[step].velocity[2] * timestep + 0.5 * sat.steps[step].acceleration[2] * timestep * timestep;
            };
        };
    };
};
...
int main() {
    ...
    float positions1[3] = { 10000.0f, 0.0f, 0.0f };
